/// <reference path="../lib/jquery-2.1.0-vsdoc.js" />
...
function AudioPlayer(options) {
    ...
    this.showCurrentLocationOnMap = (function (ctx) {
        return function (e) {
            if (navigator.geolocation) {
                if (typeof (google) === "undefined") {
                    alert("Connectivity issue!!. Could not load Google Maps.");
                }
                else {
                    navigator.geolocation.getCurrentPosition(ctx.showLocationOnMap, ctx.handleLocationError);
                }
            }
            else {
    ...
    this.showLocationOnMap = (function (ctx) {
        return function (position) {
            if (!this.map) {
                ...
                var map = this.map = new google.maps.Map(mapCanvas, mapOptions);
    ...
    this.toggleMapModal = function (e) {
        var $mapOverlay = $(".map-overlay");
        var $mapCanvas = $("#map-canvas", $mapOverlay);
    ...
    $(".close", this.$mapOverlay).off("click").on("click", this.toggleMapModal);
    $(".image.location", this.$header).off("click").on("click", this.showCurrentLocationOnMap);
}
